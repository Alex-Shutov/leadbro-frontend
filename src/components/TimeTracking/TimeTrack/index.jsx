          ...
          {employee.lastName} {employee.name} {employee.middleName}
                ...
                <div className={styles.tracking_info}>
                    <div className={styles.time}>
                        ...
                        <span>{timeSpent.hours} ч {timeSpent.minutes} мин.</span>
                    </div>
                    <div className={styles.cost}>
                        <Icon name="ruble" size={18} />
                        <span>{timeTracking.cost.toFixed(2)}</span>
                    </div>
                </div>
            </div>
            <div className={styles.rightSide}>
                <div className={styles.hours}>{formatDateOnlyHours(timeTracking.date)}</div>
                {user?.id === sender?.id && (
