...
const TimeTrackingList = ({ timeTrackings, onEdit, onDelete,onSave, onReset  }) => {
    const { user } = useUser();
    const {groupedByDate:groupedTimeTracks,isLoading:groupedLoading} = useGroupByDate(timeTrackings );
    ...
    const getActions = (data) => [
        {
            ...
            visible: data.employee.id === user.id,
        },
        {
            label: 'Удалить',
    ...
    return (
        <>
        <div ref={ref} className={cn(styles.commentList)}>
            ...
            {Boolean(timeTrackToEdit) && <EditTimeTrackModal onChange={handleEditTimeTracking} onSubmit={handleSaveTimeTracking} isOpen={Boolean(timeTrackToEdit)} timeTracking={timeTrackToEdit} onClose={handleResetTimeTrack} />}
        </>
    );
};
