...
import {formatDateWithDateAndYear, formatDateWithoutHours} from "../../../../../../utils/formate.date";
    ...
    return (
        <>
            ...
            >
                ...
                />
                <LabeledParagraph
                    textClass={service.status === statusTypes.inProgress ? styles.inProgress : styles.closed}
                    label={'Статус'}
                    text={statusTypesRu[service.status]}
                />
                {!!service.manager && <LabeledParagraph
                    label={'Ответственный'}
                    text={<ManagerCell manager={service.manager}/>}
                />}
                {!!service.command.length && <LabeledParagraph
                    label={'Участники'}
                    text={service.command.map((el)=><ManagerCell manager={el}/>)}
                />}
                ...
                {!!service.client && <LabeledParagraph
                    label={'Клиент'}
